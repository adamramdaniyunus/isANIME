...
import { getDataRandom, getDataRecomended, getDataUpComing } from '@/lib/Data'
import LoadingSkeleton from './LoadingSkeleton'
import { MdOutlineStarBorder } from 'react-icons/md'
import { useQuery } from '@tanstack/react-query'

const UpComing = ({ handleMouseEnter, handleMouseLeave }: { handleMouseEnter: (e: any, i: string) => void, handleMouseLeave: (e: any) => void }) => {
    const {
        data: animes,
        refetch,
        isFetching,
    ...
    return (
        <div>
            <div className='flex justify-between items-center'>
                <h1 className='text-white text-xl ml-4'>Upcoming</h1>
                <Link href={'/anime/upcoming'} className='text-appRed underline px-4 py-2 rounded-md'>See More</Link>
            </div>
            <div className='mx-4'>
                <Slider {...settings}>
